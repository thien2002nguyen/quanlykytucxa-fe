...
const FormRoom = ({ formAction, onBack }: FormProps) => {
  ...
  const dataRoomTypesOptions = dataRoomTypes.data?.map((item) => ({
    label: `${item.type} - ${formatVND(item.price || 0)} VND`,
    value: item._id,
  }));
  ...
  console.log("currentMultiFile: ", currentMultiFile);
      ...
      >
        <Row gutter={16}>
          ...
          <Col span={24}>
            ...
            <div className="form-item-comtainer">
              <h4 className="form-item-comtainer-title">
                Danh sách thiết bị phòng
              </h4>
              <DeviceItemComponent />
            </div>
